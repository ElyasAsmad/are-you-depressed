<script lang="ts">
	...
	async function redirectToAuthCodeFlow(clientId: string) {
		...
		params.append('redirect_uri', 'https://are-you-depressed.elyasasmad.com/callback');
...
{:else}
	<div class="mx-auto max-w-screen-xl flex flex-col items-center">
		...
		<div class="mx-auto border rounded-lg p-4 w-full mb-4">
			<h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">
				{predictionData?.message}
			</h5>
			<p class="text-lg font-semibold">
				{(predictionData?.probability ?? -1) * 100}% chance that you are depressed
			</p>
			<p class="font-normal text-gray-700 dark:text-gray-400 leading-tight">
				Suggestion:<br>{predictionData?.suggestion}
			</p>
		</div>
